...
![File transfer schematic](extra/figures/file_transfer.jpg)\
...
This is the point at which raw data are available, since it is where they are demultiplexed. Originally, our inventory had 1736 envelopes/samples indexed. Four envelopes/samples had no leaves in them, and 5 samples were the wrong species (*Taraxacum erythrospermum*), and were excluded. We also had one sample that failed to yield any reads (LS.LA.MAR.U.1); it's unclear what happened with this sample (DNA concentration, amplification looked fine - perhaps accidentally given the wrong barcodes). This gives **1726 samples available on SRA**. Note that many of these are excluded due to low coverage, etc., in subsequent steps.
...
PolyRAD output data is available via Figshare: 10.6084/m9.figshare.30640199
