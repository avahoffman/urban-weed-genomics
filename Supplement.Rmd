...
## Step 8 - `polyRAD` {#step-8---polyrad}

We used the [polyRAD package](https://github.com/lvclark/polyRAD) to call genotypes because many of our species are polyploid or have historical genome duplication. We found the following resources helpful:
...
PolyRAD takes the output from sstacks, e.g.:

```
CD.BA.AA.U.1.alleles.tsv.gz
CD.BA.AA.U.1.matches.bam
CD.BA.AA.U.1.matches.tsv.gz
CD.BA.AA.U.1.snps.tsv.gz
CD.BA.AA.U.1.tags.tsv.gz 
```

We used these files to create a RADdata object class with the `make_polyRAD_<spp>.R` script, wrapped by `polyrad_make_<spp>.sh` to submit the script to the SLURM scheduler.
...
## $F_{ST}$ - Variance among populations

We used `hierfstat::wc()` to calculate an $F_{ST}$ statistic for each species.
...
We used `hierfstat::basic.stats()` to calculate $F_{IS}$. This is equal to (expected - observed) / expected, and thus gives a good estimate of whether there are more homozygotes than expected (positive number) or more heterozygotes than expected (negative number).

```{r Fis_code, eval = FALSE, message=FALSE}
calc_fst()
```
...
```{r rbarD_table, echo=FALSE, message=FALSE, eval = FALSE}
# Not generating this table since it's in the main text
table_ <-
...
```{r ibs_1, message=FALSE}
source("R/plot_isolation_by_state.R")
```

```{r ibs_2, eval = FALSE, message=FALSE}
plot_dendrograms()
```
