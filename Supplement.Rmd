...
This directory contains files named in this convention: `01-aspera_transfer_n.txt`. These are text files containing the *names* of `fastq.gz` files that we
...
$K={1,2,3,4,5}$ (`MAXPOPS` argument). We ran each structure run with 10000 
iterations discarded as burn-in and retained 20000 iterations. For each 
combination of species and $K$, we ran Structure 5 times. These runs create files 
that look like:
...
## Global population structure: population statistics by species

We used `polyrad::calcPopDiff()` to calculate global population statistics for each species.

```{r global_stats_libs, message=FALSE}
source("R/calc_global_stats.R")
```

```{r global_stats_code, eval = FALSE, message=FALSE}
do_all_global_stats()
```

```{r global_stats_table_readlines, message=FALSE}
# CD as an example
read.csv("output/population_stats/CD_global_stats.csv")
```

## Global population structure: Structure software results
...
## Global population structure: Structure plots
...
## $F_{IS}$ - Homozygosity within population
...
## $\bar{r}_d$ - Linkage disequilibrium
