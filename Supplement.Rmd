...
We tested the relationship between urbanness and the extent of admixture by running a correlation test between percent impervious surface and $1 - sd(K_i)$, where $sd(K_i)$ represents the standard deviation of the cluster assignment values for any given individual. We used the `cor.test()` function.
...
```{r snmf_2, message=FALSE, fig.cap = "Ancestry coefficients obtained using `snmf()`", fig.height=9}
...
We performed heirarchical analysis of molecular variance (AMOVA; using GenoDrive) based on the Rho-statistics, which is based on a Ploidy independent Infinite Allele Model.
...
We used [GenoDive v. 3.0.6](https://doi.org/10.1111/1755-0998.13145) to calculate pairise $\rho$ (rho) among cities within species. We used the following script to clean up the results.
...
```{r rho_table_readlines, message=FALSE}
# LS as an example
...
table_ <- readr::read_csv("output/population_stats/rbarD.csv")
knitr::kable(
  table_,
  format = "simple",
  caption = "rbarD statistics for species and cities in this study."
)
```
...
Note that for this analysis, we treated each *sampling site* as a distinct location. There would not be enough power to do a distance matrix among 3-5 cities.
...
table_ <- readr::read_csv("output/IBD/isolation-by-distance-mantel-test.csv")
knitr::kable(
  table_,
  format = "simple",
  caption = "Statistics from running 9999 permutations via mantel test."
)
```

We also repeated this within city. Code for generating stats from the Mantel test can be found in the source code below.

```{r ibd_4, eval = FALSE}
source("R/isolation_by_distance.R")
```

```{r ibd_5, echo = FALSE, message=FALSE}
table_ <- readr::read_csv("output/IBD/isolation-by-distance-within-city-mantel-test.csv")
knitr::kable(
  table_,
  format = "simple",
  caption = "Statistics from running 9999 permutations via mantel test, limited to within city."
)
```
...
```{r ibd_2, eval = FALSE, message=FALSE}
# Run for "nlcd_urban_pct" which is the percent urban cover of a site within city.
extract_ibe_stats_and_plots()

# Across all cities
extract_ibe_stats_and_plots(TRUE, "BA")
extract_ibe_stats_and_plots(TRUE, "PX")
extract_ibe_stats_and_plots(TRUE, "LA")
extract_ibe_stats_and_plots(TRUE, "BO")
extract_ibe_stats_and_plots(TRUE, "MN")
```

```{r ibd_3, eval = FALSE, message=FALSE}
# Run for other vars..
...
extract_ibe_stats_and_plots(env_var_to_use = "airtemp_1.2m_Jul_12pm")
extract_ibe_stats_and_plots(env_var_to_use = "soiltemp_2.5cm_Jun_12pm")
extract_ibe_stats_and_plots(env_var_to_use = "airtemp_1.2m_Jun_12pm")
extract_ibe_stats_and_plots(env_var_to_use = "soiltemp_2.5cm_Aug_12pm")
extract_ibe_stats_and_plots(env_var_to_use = "airtemp_1.2m_Aug_12pm")
extract_ibe_stats_and_plots(env_var_to_use = "RH_1.2m_Jul_12pm")

extract_ibe_stats_and_plots(env_var_to_use = "airtemp_1.2m_Jul_12pm")
extract_ibe_stats_and_plots(TRUE, "BA", env_var_to_use = "airtemp_1.2m_Jul_12pm")
extract_ibe_stats_and_plots(TRUE, "PX", env_var_to_use = "airtemp_1.2m_Jul_12pm")
extract_ibe_stats_and_plots(TRUE, "LA", env_var_to_use = "airtemp_1.2m_Jul_12pm")
extract_ibe_stats_and_plots(TRUE, "BO", env_var_to_use = "airtemp_1.2m_Jul_12pm")
extract_ibe_stats_and_plots(TRUE, "MN", env_var_to_use = "airtemp_1.2m_Jul_12pm")
```
