...
PAs_all <-
  rbind(
    tibble(spp = "CD", city = c("BA", "LA", "PX"), PA = PAs_CD %>% rowSums(), NPA = NPAs_CD %>% rowSums()),
    tibble(spp = "DS", city = c("BA", "BO", "MN", "PX"), PA = PAs_DS %>% rowSums(), NPA = NPAs_DS %>% rowSums()),
    tibble(spp = "EC", city = c("BA", "LA", "PX"), PA = PAs_EC %>% rowSums(), NPA = NPAs_EC %>% rowSums()),
    tibble(spp = "LS", city = c("BA", "BO", "LA", "MN", "PX"), PA = PAs_LS %>% rowSums(), NPA = NPAs_LS %>% rowSums()),
    tibble(spp = "PA", city = c("BA", "BO", "LA", "MN", "PX"), PA = PAs_PA %>% rowSums(), NPA = NPAs_PA %>% rowSums()),
    tibble(spp = "TO", city = c("BA", "BO", "LA", "MN", "PX"), PA = PAs_TO %>% rowSums(), NPA = NPAs_TO %>% rowSums())
)

PAs_all <- PAs_all %>% mutate(Percent_private_alleles = PA/(PA+NPA)*100)
