...
correct_structure_files <- function(spp_) {
  ...
  struct_ <-
    struct_ %>%
    mutate(pop = as.numeric(as.factor(
      str_replace(struct_$SampleName, "..$", "")
    ))) %>%
    arrange(pop)
  write_delim(struct_,
              paste0(infile_stem, "_sitesaspops.structure"),
              delim = "\t")
...
get_gen_dist_matrix <- function(spp_) {
  ...
  mat_ <- read_delim(paste0(
    "output/genodive/",
    spp_,
    "_genetic_distance_bysite_rho.txt"
  ))
...
get_env_dist_matrix_list <- function(spp_) {
  ...
  site_info <-
    ...
    unite("site_abbv", city_abbv, site_abbv, management_type, sep = ".") %>%
    mutate(pop = paste0(spp_, ".", site_abbv)) %>%
    rename(soiltemp_Apr = soiltemp_2.5cm_Apr_12pm, soiltemp_Jul = soiltemp_2.5cm_Jul_12pm)
...
make_mmrr_plot <- function() {
  ...
  if (!file.exists("output/MMRR/MMRR_total_overall_models.csv")) {
    ...
    readr::write_csv(p_vals_overall_models,
                     "output/MMRR/MMRR_total_overall_models.csv")
  } else {
  ...
  heat_plot <- function(in_dat, params = T) {
    plot_ <-
      ggplot() +
      geom_tile(data = in_dat , aes(x = Species, y = var, fill = estimate)) +
      geom_text(
        data = in_dat ,
        size = 2,
    ...
    plot_ <-
      ...
      geom_tile(data = in_dat %>% filter(spp != "EC"),
  ...
  ggsave(
    paste0("figures/Fig7_MMRR/MMRR_total.png"),
    dpi = "print",
    height = 80,
    width = 112,
    units = "mm"
...
make_mmrr_plot_bycity_ <- function() {
  ...
  p_vals_overall_models$`p adjusted` <- round(c(
    p.adjust(p_vect[1:3], method = "BH"),
    #CD
    p.adjust(p_vect[4:7], method = "BH"),
    #DS
    p.adjust(p_vect[8:10], method = "BH"),
    #EC
    p.adjust(p_vect[11:15], method = "BH"),
    #LS
    p.adjust(p_vect[16:19], method = "BH"),
    #PA
  ...
  readr::write_csv(p_vals_overall_models,
                   "output/MMRR/MMRR_splitbycity_overall_models.csv")
  
  mmrr_plot_city_ <-
    mmrr_city_ %>% filter(var != "F-Statistic:" &
                            var != "F p-value:" & var != "Intercept") %>%
    mutate(
      estimate = case_when(p > 0.05 ~ NA, TRUE ~ estimate),
      facet_ = case_when(
  ...
  env_plot_ <-
    ggplot(mmrr_plot_city_ %>% filter(facet_ == "Environment"),
           aes(x = spp_city, y = var)) +
  ...
  dist_plot_ <-
    ggplot(mmrr_plot_city_ %>% filter(facet_ == "Distance"),
           aes(x = spp_city, y = var)) +
    geom_tile(aes(fill = estimate)) +
    geom_text(aes(label = estimate_rounded)) +
    scale_fill_gradient2(
  ...
  r2_plot_ <-
    ggplot(mmrr_plot_city_ %>% filter(var == "R-Squared"),
           aes(x = spp_city, y = var)) +
    geom_tile(aes(fill = estimate)) +
    geom_text(aes(label = estimate_rounded)) +
    scale_fill_gradient2(
  ...
  ggsave(
    paste0("figures/Fig7_MMRR/MMRR_splitbycity.png"),
    dpi = "print",
    height = 4,
    width = 12,
    units = "in"
